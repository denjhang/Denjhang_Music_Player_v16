...
static void generate_adpcm(okim6295_state *chip, okim_voice *voice, DEV_SMPL *buffer, UINT32 samples, int m_voice_ofs, int64_t smplIncr)
{
	UINT32 i;
        ...
        for (i = 0; i < OKIM6295_VOICES; i++) {
            ...
            if (m_voice_ofs>=0) {
                if (chip->voice[i].playing && chip->voice[i].volume && !(chip->voice[i].Muted)) {
                    ...
                    vgm_last_note[m_voice_ofs+i]=freq*440.0f/22050.0f;
                    vgm_last_instr[m_voice_ofs+i]=m_voice_ofs+i;

                    vgm_last_vol[m_voice_ofs+i]=1;
                }
            }
            //YOYOFR
