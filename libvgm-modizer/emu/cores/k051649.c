...
static void k051649_update(void *param, UINT32 samples, DEV_SMPL **outputs)
{
    ...
    //TODO:  MODIZER changes start / YOYOFR
    // Register scope slot for SCC (5 waveform channels)
    ...
    // Pre-compute step for each channel
    UINT32 steps[5];
    for (j = 0; j < 5; j++) {
        if (voice[j].frequency > 8 && !voice[j].Muted)
            steps[j] = (UINT32)(((INT64)info->mclock * (1 << FREQ_BITS)) / (float)((voice[j].frequency + 1) * info->rate / 2.0f) + 0.5f);
        else
            steps[j] = 0;
    }

    // Audio generation: loop by sample, advance all channels per sample
    // (restructured from per-channel outer loop to enable shared-ofs scope write)
    //TODO:  MODIZER changes start / YOYOFR — shared-ofs scope buffer write
    for (i = 0; i < samples; i++)
    {
        DEV_SMPL mix = 0;
        DEV_SMPL ch_out[5];
        ...
        // shared-ofs scope buffer write: all 5 channels share one pointer
        if (m_voice_ofs >= 0) {
            ...
            for (;;) {
                for (int jj = 0; jj < 5; jj++) {
                    m_voice_buff[m_voice_ofs + jj]
                        [(ofs_start >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT) & (SOUND_BUFFER_SIZE_SAMPLE * 4 * 2 - 1)]
                        = LIMIT8(ch_out[jj] >> 2);
                }
    ...
    //YOYOFR legacy: note/instr tracking
    if (m_voice_ofs >= 0) {
        for (int jj = 0; jj < 5; jj++) {
            if (steps[jj] && voice[jj].key && voice[jj].volume) {
                double freq=(double)(voice[jj].frequency);
                if (freq) {
                    freq=(double)(info->mclock)*2/32/(freq+1);
                    vgm_last_note[m_voice_ofs+jj]=freq;
                    vgm_last_instr[m_voice_ofs+jj]=m_voice_ofs+jj;
                    vgm_last_vol[m_voice_ofs+jj]=1;
                }
            }
        }
    }
