...
static void ws_audio_update(void* info, UINT32 length, DEV_SMPL** buffer)
{
    ...
    ScopeChipSlot *scope_slot = scope_register_chip("WSWAN", 0, 4, m_voice_current_samplerate);
    if (scope_slot && scope_slot->num_groups == 0) {
        scope_slot->groups[0] = (ScopeChGroup){SCOPE_CH_DAC, 4, 0, "W"};
    ...
    if (m_voice_ofs>=0)
        for (int ii=0;ii<4;ii++) {
            if ( !(chip->ws_audio[ii].Muted) ) {
                ...
                if (vol) {
                    // Correct WSWAN frequency: clock / (16 * (2048 - period))
                    UINT16 period_reg = (chip->ws_ioRam[0x81 + ii*2] << 8) | chip->ws_ioRam[0x80 + ii*2];
                    if (period_reg < 2048)
                        vgm_last_note[ii+m_voice_ofs] = 3072000.0 / 16.0 / (2048.0 - (period_reg & 0x7FF));
