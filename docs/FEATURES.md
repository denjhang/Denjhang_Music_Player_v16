...
11. [AY8910 (PSG) VGM 播放器模块](#11-ay8910-psg-vgm-播放器模块)
12. [SPFM Light 硬件管理模块](#12-spfm-light-硬件管理模块)
13. [统一 VGM 播放引擎](#13-统一-vgm-播放引擎)
14. [配置管理模块](#14-配置管理模块)
15. [VGM 文件解析模块](#15-vgm-文件解析模块)
16. [可视化模块（Modizer）](#16-可视化模块modizer)
17. [第三方库](#17-第三方库)
18. [构建系统](#18-构建系统)
19. [版本演进](#19-版本演进)
...
## 11. AY8910 (PSG) VGM 播放器模块

### 文件：`src/windows/ay8910/ay8910_window.h` / `src/windows/ay8910/ay8910_window.cpp`

### 11.1 功能概览

AY8910 (PSG) 是 General Instrument 的可编程声音发生器，3 个方波通道 + 1 个噪音通道 + 1 个包络发生器。广泛用于 MSX、ZX Spectrum、Intellivision 等平台。

### 11.2 寄存器影子跟踪

- 寄存器 R0-R13 完整影子
- Key-on/key-off 检测：音量从 0→非0 = key-on，非0→0 = key-off
- 3 旋律通道周期 + 音量独立跟踪
- 噪音通道频率/类型跟踪

### 11.3 钢琴键盘

- 3 旋律通道（Tone A/B/C）+ 噪音通道
- VGM blendKey 着色（音量→颜色渐变）
- 噪音通道频率计算与音符映射
- Portamento 滑音可视化（跨帧频率变化跟踪）

### 11.4 颜色系统

- 4 通道独立颜色（Tone A/B/C + Noise）
- 自定义颜色 UI（ColorEdit4），INI 持久化

### 11.5 硬件写入

- AY8910 使用 `spfm_write_reg(slot, port, reg, data)` 单次写入
- **必须用单次写入模式**，双写地址锁存模式（先写地址再写数据）是错误的

---

## 12. SPFM Light 硬件管理模块

### 文件：`src/windows/spfm/spfm_manager.h` / `src/windows/spfm/spfm_manager.cpp`

### 12.1 功能概览

统一管理 SPFM Light 硬件的连接、Slot 配置和芯片类型切换。

### 12.2 SPFM Manager 窗口
...
- 预设保存到 `bin/slot_presets.ini`
- 下次打开相同组合的 VGM 自动恢复 Slot 配置
...
### 文件：`src/core/vgm_sync.h` / `src/core/vgm_sync.cpp`

### 13.1 架构

单线程解析 + 多芯片分发。只有一个统一的播放线程运行，解析所有 VGM opcode 并根据芯片类型分发到对应窗口的回调函数。三个窗口（YM2413/AY8910/SN76489）不再各自运行独立播放线程，消除了多窗口节奏不同步问题。

### 13.2 回调注册机制

```cpp
// vgm_sync.h
typedef uint8_t (*ChipStateUpdateFn)(uint8_t reg, uint8_t data);  // 更新 shadow state
typedef void    (*ChipHwWriteFn)(uint8_t reg, uint8_t data);      // 硬件写入
typedef void    (*ChipFlushFn)(void);                              // 可选 flush
typedef void    (*SnCmdFn)(uint8_t cmd, uint8_t data);            // SN76489 命令

void RegisterChipWriter(int chipType, ChipStateUpdateFn, ChipHwWriteFn, ChipFlushFn);
void RegisterSnHandler(SnCmdFn);
```

每个窗口在 `Init()` 时注册回调：
- YM2413：`UpdateYM2413State` + `ym2413_write_reg` + `safe_flush`
- AY8910：`UpdateAY8910State` + `ay8910_write_reg` + `safe_flush`
- SN76489：`SnCmdHandler`（封装完整 0x50/0x30 逻辑，含 T6W28/latch/mute/PeriodicNoiseFix）

### 13.3 统一播放线程

- VGM 文件加载（支持 VGZ gzip 解压），完整 header 解析
- Opcode 分发：0x51→YM2413, 0xA0→AY8910, 0x50/0x30→SN76489, 控制指令统一处理
- QPC+Sleep(1) 高精度定时，每帧最大 44100 samples 安全上限
- `last` QPC 计数器在循环前初始化（修复首次迭代瞬间处理全部文件的 bug）

### 13.4 共享进度

```cpp
void SetCurrentSamples(uint32_t samples);   // 统一线程写入
uint32_t GetCurrentSamples();               // 所有窗口 UI 读取
uint32_t GetTotalSamples();                 // 总时长
```

每个窗口 `Update()` 中同步：`s_vgmCurrentSamples = VGMSync::GetCurrentSamples()`

### 13.5 窗口改造

- `Start/Stop/PauseVGMPlayback` 委托给 `VGMSync::Start/Stop/PauseUnifiedPlayback`
- `Update()` 同步逻辑改为仅同步本地 UI 状态，不再启动自己的线程
- `Shutdown()` 移除本地线程等待代码
- 硬件写入函数从 `static` 改为 namespace 函数（供统一线程回调调用）
...
| **v16.0** | **2026-04-28~05-06** | **DockSpace 多窗口、SN76489、Gigatron、YM2413 OPLL、AY8910 PSG、SPFM 硬件管理、统一 VGM 播放引擎** |
